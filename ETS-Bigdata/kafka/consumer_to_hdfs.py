...
FLUSH_INTERVAL   = 30    # flush lebih cepat supaya HDFS segera terisi
MAX_BUFFER_SIZE  = 5     # buffer kecil agar file cepat dibuat
...
def consume_api():
    ...
    for message in consumer:
        ...
        with buffer_lock:
            ...
            should_flush = len(api_buffer) >= MAX_BUFFER_SIZE
        if should_flush:
            flush_buffers()
...
def consume_rss():
    ...
    for message in consumer:
        ...
        with buffer_lock:
            ...
            should_flush = len(rss_buffer) >= MAX_BUFFER_SIZE
        if should_flush:
            flush_buffers()
